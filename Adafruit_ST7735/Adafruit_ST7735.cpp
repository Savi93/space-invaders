...
void Adafruit_ST7735::commonInit(uint8_t *cmdList)
{
    ...
    lcdPort.format(8,0);
    lcdPort.frequency(18000000);     // Lets try 4MHz
...
#define OffsetBPP           28
    ...
    if (!Image) {
        printf("TEST error file not found");
...
int Adafruit_ST7735::DrawBitmapFile(const char *Name_BMP)
{
    ...
    {
    #ifdef TFT_DEBUG    
        printf("Not supported compress : %d\r\n",compress);
    #endif
        return -4;    
    }
...
#ifdef TFT_DEBUG    
    printf("RESOL : %d col, %d X %d",colbits,imgw,imgh);
#endif    
